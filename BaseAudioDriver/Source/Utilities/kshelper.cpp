...
Abstract:
    ...
    Helper functions for base audio driver
--*/
...
#pragma code_seg("PAGE")
NTSTATUS
BaseAudioDriverPropertyDispatch
(
    _In_ PPCPROPERTY_REQUEST PropertyRequest
)
/*++
    Handles and dispatches a BASEAUDIODRIVER_PROPERTY_ITEM.

    Use this as the property handler only if the property item is a
    BASEAUDIODRIVER_PROPERTY_ITEM.
--*/
{
    PAGED_CODE();

    BASEAUDIODRIVER_PROPERTY_ITEM* item = (BASEAUDIODRIVER_PROPERTY_ITEM*)PropertyRequest->PropertyItem;
