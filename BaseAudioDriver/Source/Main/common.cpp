    ...
    public CUnknown    
{
    private:
        ...
        PCBaseAudioDriverHW   m_pHW;                  // Virtual Base Audio Driver HW object
    ...
    public:
        ...
        ( 
            ...
            _In_reads_opt_(cPropertyCount) const BASEAUDIODRIVER_DEVPROPERTY   * pProperties,
...
NTSTATUS BaseAudioDriverIoSetDeviceInterfacePropertyDataMultiple
(
    _In_ PUNICODE_STRING                                        SymbolicLinkName,
    _In_ ULONG                                                  cPropertyCount,
    _In_reads_opt_(cPropertyCount) const BASEAUDIODRIVER_DEVPROPERTY        *pProperties
...
--*/
{
    ...
    // Initialize HW.
    // 
    m_pHW = new (POOL_FLAG_NON_PAGED, BASEAUDIODRIVER_POOLTAG)  CBaseAudioDriverHW;
    if (!m_pHW)
    {
        DPF(D_TERSE, ("Insufficient memory for Base Audio Driver HW"));
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    }
...
--*/
{
    ...
    //
    ntStatus = BaseAudioDriverIoSetDeviceInterfacePropertyDataMultiple(AudioSymbolicLinkName, cPropertyCount, pProperties);

    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("CreateAudioInterfaceWithProperties: BaseAudioDriverIoSetDeviceInterfacePropertyDataMultiple(...): failed, 0x%x", ntStatus)),
        Done);
...
)
{
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology || NULL == unknownMiniTopo)
    {
        ...
        // Install Base Audio Driver topology miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->TopoName, // make sure this name matches with BASEAUDIODRIVER.<TopoName>.szPname in the inf's [Strings] section
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownWave || NULL == unknownMiniWave)
    {
        ...
        // Install Base Audio Driver wave miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->WaveName, // make sure this name matches with BASEAUDIODRIVER.<WaveName>.szPname in the inf's [Strings] section
